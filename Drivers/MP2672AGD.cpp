...
/*
* Charger_GetFaults
* Reads REG04 (fault register) and unpacks all fault bits into the
* provided ChargerFault_t struct.
* Returns true on successful I2C read.
*/
bool Charger_GetFaults(ChargerFault_t *faults) {
    if (faults == NULL) return false;
    uint8_t raw;
    if (!Charger_Read_Reg(MP2672_REG04, &raw)) return false;

    faults->watchdogExpired = (raw & REG04_WD_FAULT) != 0;
    faults->inputOVP = (raw & REG04_INPUT_FAULT) != 0;
    faults->thermalShutdown = (raw & REG04_THERMSD_FAULT) != 0;
    faults->safetyTimerExp = (raw & REG04_TIMER_FAULT) != 0;
    faults->battOVP = (raw & REG04_BAT_FAULT) != 0;
    faults->ntcFault = (NtcFault_t)((raw & REG04_NTC_FAULT_MASK) >> REG04_NTC_FAULT_SHIFT);
    return true;
}
